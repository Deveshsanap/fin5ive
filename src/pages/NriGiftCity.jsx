...
import { 
  ...
  ChevronDown, ChevronUp, Download, Building, Plane, Loader2
} from 'lucide-react';
...
const NriGiftCity = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phoneNumber: '',
    email: '',
    interest: 'Inbound Funds (Investing in India)'
  });
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', country: '', phoneNumber: '', email: '', interest: 'Inbound Funds (Investing in India)' });
    } catch (error) {
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-[fadeIn_0.3s_ease-out]">
            ...
            <form className="p-8 space-y-4" onSubmit={handleModalSubmit}>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-xl mt-4 flex justify-center items-center transition shadow-lg ${isSubmitting ? 'bg-gray-400' : 'bg-[#FF6600] hover:bg-[#e55c00]'}`}>
                {isSubmitting ? <Loader2 className="w-5 h-5 mr-2 animate-spin"/> : <>Schedule Call <Send className="w-5 h-5 ml-2" /></>}
              </button>
            </form>
          </div>
        </div>
