...
import { 
  ...
  Calculator, Globe, Scale, RefreshCw, FileSearch, CreditCard, Loader2
} from 'lucide-react';
...
const NriTax = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    country: '',
    phoneNumber: '',
    interest: 'Income Tax Return (ITR) Filing'
  });
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', email: '', country: '', phoneNumber: '', interest: 'Income Tax Return (ITR) Filing' });
    } catch (error) {
  ...
  const [saleValue, setSaleValue] = useState(20000000); 
  const [purchaseValue, setPurchaseValue] = useState(8000000); 
            ...
            <form className="p-8 space-y-4" onSubmit={handleModalSubmit}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={formData.country} onChange={(e) => setFormData({...formData, country: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#FF6600] outline-none" placeholder="e.g. USA, UAE" />
                </div>
                <div>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-xl mt-4 flex justify-center items-center transition shadow-lg ${isSubmitting ? 'bg-gray-400' : 'bg-[#FF6600] hover:bg-[#e55c00]'}`}>
                {isSubmitting ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <>Get Tax Assistance <Send className="w-5 h-5 ml-2" /></>}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-w-5xl mx-auto text-gray-800 flex flex-col md:flex-row">
            <div className="p-8 md:p-12 md:w-1/2 border-b md:border-b-0 md:border-r border-gray-200 bg-slate-50">
              <div className="mb-10">
                ...
                <input type="range" min="5000000" max="100000000" step="1000000" value={saleValue} onChange={(e) => setSaleValue(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
              </div>
              <div className="mb-4">
                <div className="flex justify-between items-center mb-4">
                ...
                <input type="range" min="1000000" max="90000000" step="1000000" value={purchaseValue} onChange={(e) => setPurchaseValue(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
