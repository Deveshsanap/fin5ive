...
import { 
  ...
  Umbrella, Building, Calculator, CheckCircle2, Loader2
} from 'lucide-react';
...
const NriDomestic = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    country: '',
    phoneNumber: '',
    interest: 'Mutual Funds (SIP / Lumpsum)'
  });
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', email: '', country: '', phoneNumber: '', interest: 'Mutual Funds (SIP / Lumpsum)' });
    } catch (error) {
          ...
          <div className="bg-slate-50 rounded-[2.5rem] shadow-xl border border-gray-200 overflow-hidden max-w-5xl mx-auto flex flex-col md:flex-row">
            <div className="p-8 md:p-12 md:w-3/5 border-b md:border-b-0 md:border-r border-gray-200 bg-white">
              <div className="flex bg-slate-100 rounded-xl p-1 mb-8">
                <button onClick={() => setCalcType('SIP')} className={`flex-1 py-3 rounded-lg font-bold text-sm transition-all ${calcType === 'SIP' ? 'bg-[#003366] text-white shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>Monthly SIP</button>
                <button onClick={() => setCalcType('LUMPSUM')} className={`flex-1 py-3 rounded-lg font-bold text-sm transition-all ${calcType === 'LUMPSUM' ? 'bg-[#FF6600] text-white shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>One-Time Lumpsum</button>
              </div>
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <label className="font-bold text-[#003366]">{calcType === 'SIP' ? 'Monthly Investment' : 'Total Investment'} (INR)</label>
                  <div className="bg-slate-50 border border-gray-200 px-4 py-2 rounded-lg font-bold text-[#FF6600] shadow-sm">{formatINR(investmentAmount)}</div>
                </div>
                <input type="range" min={calcType === 'SIP' ? 5000 : 100000} max={calcType === 'SIP' ? 500000 : 50000000} step={calcType === 'SIP' ? 5000 : 100000} value={investmentAmount} onChange={(e) => setInvestmentAmount(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="range" min="6" max="25" step="1" value={expectedReturn} onChange={(e) => setExpectedReturn(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
                </div>
                <div>
                  <div className="flex justify-between items-center mb-4">
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div key={index} className="bg-white border border-gray-200 rounded-2xl overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md">
                <button onClick={() => setActiveFaq(activeFaq === index ? null : index)} className={`w-full flex justify-between items-center p-6 md:p-8 transition text-left ${activeFaq === index ? 'bg-white' : 'hover:bg-gray-50'}`}>
