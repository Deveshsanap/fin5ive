...
const MsmeFunding = () => {
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      await createLead({
        ...
        service: `MSME Funding - ${formData.fundingType}`,
        message: `Turnover: ${formData.turnover}`
      });
      toast.success("Request Sent! Our advisory team will contact you shortly.");
      setIsModalOpen(false);
      setFormData({ companyName: '', contactPerson: '', phoneNumber: '', email: '', turnover: 'Up to ₹5 Crores', fundingType: 'New Machinery Purchase' });
    } catch (error) {
  ...
  const fundingTypes = {
    MACHINERY: { title: "Machinery Loans", icon: <Settings className="w-6 h-6" />, desc: "Upgrade your manufacturing unit with state-of-the-art equipment. We arrange up to 100% funding for imported or domestic machinery.", points: ["Up to 100% Invoice Value funding", "Subsidies under TUFS & State Schemes", "Longer repayment tenures (up to 7 years)", "Fast processing with minimal collateral"] },
    WORKING_CAP: { title: "Working Capital (CC/OD)", icon: <Coins className="w-6 h-6" />, desc: "Maintain seamless business cycles. We structure Cash Credit and Overdraft facilities against your inventory, receivables, or property.", points: ["Limits from ₹50 Lakhs to ₹25 Crores", "Optimized interest rates", "CGTMSE (Collateral-free) options up to ₹5 Cr", "Enhancement of existing limits"] },
    SOLAR: { title: "Solar & Green Finance", icon: <Sun className="w-6 h-6" />, desc: "Drastically reduce your OPEX by transitioning to renewable energy. Specialized funding for industrial rooftop solar installations.", points: ["Up to 80% project cost funding", "Dedicated green-energy interest concessions", "Quick ROI realization (3-4 years)", "Assistance with capital subsidies"] },
    FACTORY: { title: "Factory Construction", icon: <Factory className="w-6 h-6" />, desc: "End-to-end project finance for acquiring industrial land and constructing your dream manufacturing facility.", points: ["Land + Construction composite loans", "Structuring moratorium periods during construction", "Integration with state industrial policies", "Optimal debt-equity structuring"] }
  };
  ...
  return (
    <div className="bg-white relative">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-[fadeIn_0.3s_ease-out]">
            ...
            <form className="p-8 space-y-4" onSubmit={handleModalSubmit}>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-lg mt-4 flex justify-center items-center transition shadow-lg ${isSubmitting ? 'bg-gray-400' : 'bg-finOrange hover:bg-finOrange-dark'}`}>
                {isSubmitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Submitting...</> : <>Book Free Consultation <Send className="w-5 h-5 ml-2" /></>}
              </button>
            </form>
          </div>
        </div>
      ...
      <div className="bg-finBlue text-white py-24 relative overflow-hidden">
        <div className="absolute top-0 right-0 -mr-20 -mt-20 opacity-10"><Factory className="w-96 h-96 animate-[pulse_10s_ease-in-out_infinite]" /></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl">
            <div className="flex items-center space-x-2 text-finOrange font-bold tracking-wider uppercase text-sm mb-4"><Settings className="w-5 h-5 animate-[spin_4s_linear_infinite]" /><span>MSME Financial Advisory</span></div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold mb-6 leading-tight">Empowering MSMEs Through <span className="text-finOrange">Smart Funding Solutions.</span></h1>
            <p className="text-xl text-gray-300 mb-8 leading-relaxed">We navigate the complexities of business funding, government subsidies, and compliance certifications so you can focus on scaling your manufacturing unit.</p>
            <div className="flex flex-wrap gap-3 mb-10">
              {['Working Capital', 'Machinery Loans', 'Factory Finance', 'ZED Certification', 'Govt Subsidies'].map((tag, i) => (<span key={i} className="bg-white/10 border border-white/20 text-white px-4 py-1.5 rounded-full text-sm font-medium backdrop-blur-sm">{tag}</span>))}
            </div>
            <button onClick={() => setIsModalOpen(true)} className="bg-finOrange hover:bg-finOrange-light text-white font-bold py-4 px-8 rounded-lg transition duration-300 shadow-lg shadow-finOrange/30 flex justify-center items-center">Evaluate Your Funding Eligibility <ArrowRight className="ml-2 w-5 h-5" /></button>
          </div>
        </div>
      </div>
      ...
      <section className="py-24 bg-slate-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16"><span className="text-finOrange font-bold tracking-wider uppercase text-sm mb-4 block">Financial Engineering</span><h2 className="text-3xl font-bold text-finBlue mb-4">MSME Loan & Subsidy Calculator</h2><p className="text-gray-600 max-w-2xl mx-auto">See how proper structuring and government interest subvention schemes can drastically reduce your monthly outflow.</p></div>
          <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden max-w-5xl mx-auto flex flex-col md:flex-row">
            <div className="p-8 md:p-12 md:w-1/2 border-b md:border-b-0 md:border-r border-gray-200">
              <div className="mb-8"><div className="flex justify-between items-center mb-4"><label className="font-bold text-finBlue">Loan Amount</label><div className="bg-slate-50 border border-gray-200 px-4 py-2 rounded-lg font-bold text-finBlue">{formatINR(loanAmount)}</div></div><input type="range" min="1000000" max="250000000" step="1000000" value={loanAmount} onChange={(e) => setLoanAmount(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-finOrange" /></div>
              <div className="mb-8"><div className="flex justify-between items-center mb-4"><label className="font-bold text-finBlue">Tenure (Years)</label><div className="bg-slate-50 border border-gray-200 px-4 py-2 rounded-lg font-bold text-finBlue">{tenureYears} Years</div></div><input type="range" min="1" max="15" step="1" value={tenureYears} onChange={(e) => setTenureYears(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-finOrange" /></div>
              <div className="grid grid-cols-2 gap-6">
                <div><div className="flex justify-between items-center mb-4"><label className="font-bold text-gray-600 text-sm">Bank ROI</label><span className="font-bold text-finBlue">{interestRate}%</span></div><input type="range" min="7.5" max="15" step="0.1" value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-finBlue" /></div>
                <div><div className="flex justify-between items-center mb-4"><label className="font-bold text-finOrange text-sm">Govt Subsidy</label><span className="font-bold text-finOrange bg-orange-50 px-2 py-1 rounded">{subsidyRate}%</span></div><input type="range" min="0" max="7" step="1" value={subsidyRate} onChange={(e) => setSubsidyRate(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-finOrange" /></div>
              </div>
            </div>
            <div className="p-8 md:p-12 md:w-1/2 bg-finBlue text-white relative overflow-hidden flex flex-col justify-center">
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex justify-between items-center text-gray-300"><span>Standard Bank EMI</span><span className="font-bold text-xl line-through">{formatINR(standardEMI)}</span></div>
                <div className="p-6 bg-white/10 rounded-xl border border-white/20 backdrop-blur-sm"><p className="text-finOrange font-bold text-sm tracking-widest uppercase mb-1">Effective Subsidized EMI</p><p className="text-4xl font-extrabold text-white">{formatINR(subsidizedEMI)}</p><p className="text-gray-300 text-xs mt-2 italic">At effective interest rate of {Math.max(0, interestRate - subsidyRate)}%</p></div>
                <div className="pt-4 border-t border-white/20"><p className="text-green-400 font-bold text-sm tracking-widest uppercase mb-1">Total Savings on Interest</p><p className="text-3xl font-extrabold text-green-400">{formatINR(totalSavings)}</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16"><h2 className="text-3xl font-bold text-finBlue mb-4">Tailored MSME Funding Suites</h2><p className="text-gray-600 max-w-2xl mx-auto">From daily operations to greenfield expansions, we structure the exact credit facility your business needs.</p></div>
          <div className="max-w-6xl mx-auto bg-slate-50 border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="flex flex-wrap md:flex-nowrap border-b border-gray-200 bg-white">
              {Object.keys(fundingTypes).map((key) => (<button key={key} onClick={() => setActiveTab(key)} className={`flex-1 py-5 px-2 text-center font-bold text-sm md:text-base transition-colors flex flex-col md:flex-row items-center justify-center gap-2 ${activeTab === key ? 'border-b-4 border-finOrange text-finBlue bg-slate-50' : 'text-gray-400 hover:bg-gray-50 border-b-4 border-transparent'}`}>{fundingTypes[key].icon} <span className="hidden md:inline">{fundingTypes[key].title}</span></button>))}
            </div>
            <div className="p-8 md:p-12">
              <div className="flex items-center mb-6"><div className="p-4 bg-finOrange/10 rounded-xl text-finOrange mr-6 hidden md:block">{fundingTypes[activeTab].icon}</div><div><h3 className="text-2xl font-bold text-finBlue mb-2">{fundingTypes[activeTab].title}</h3><p className="text-gray-600 leading-relaxed text-lg">{fundingTypes[activeTab].desc}</p></div></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 pt-8 border-t border-gray-200">{fundingTypes[activeTab].points.map((point, i) => (<div key={i} className="flex items-start bg-white p-4 rounded-xl border border-gray-100 shadow-sm"><CheckCircle className="w-5 h-5 text-finOrange mt-0.5 mr-3 flex-shrink-0" /><span className="text-gray-700 font-medium">{point}</span></div>))}</div>
            </div>
          </div>
        </div>
      </section>

      {/* Strategic Advisory Grid */}
      <section className="py-24 bg-slate-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16"><h2 className="text-3xl font-bold mb-4">The Execution Roadmap</h2><p className="text-gray-400 max-w-2xl mx-auto">From initial assessment to final disbursal, we project-manage your entire funding lifecycle.</p></div>
      ...
      <section className="py-24 bg-slate-50 border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16"><h2 className="text-3xl font-bold text-finBlue mb-4">Frequently Asked Questions</h2><p className="text-gray-600">Clearing up common questions about MSME funding and government schemes.</p></div>
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div key={index} className="bg-white border border-gray-200 rounded-lg overflow-hidden transition-all duration-300 shadow-sm">
                <button onClick={() => setActiveFaq(activeFaq === index ? null : index)} className="w-full flex justify-between items-center p-6 hover:bg-gray-50 transition text-left"><span className="font-bold text-finBlue text-lg pr-4">{faq.question}</span>{activeFaq === index ? <ChevronUp className="w-5 h-5 text-finOrange flex-shrink-0" /> : <ChevronDown className="w-5 h-5 text-gray-400 flex-shrink-0" />}</button>
                <div className={`px-6 overflow-hidden transition-all duration-300 ${activeFaq === index ? 'max-h-40 py-6 border-t border-gray-100' : 'max-h-0 py-0'}`}><p className="text-gray-600 leading-relaxed">{faq.answer}</p></div>
              </div>
            ))}
          </div>
        </div>
