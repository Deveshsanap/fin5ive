...
import { 
  ...
  Calculator, PieChart, Coins, Loader2
} from 'lucide-react';
...
const NriRealEstate = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    country: '',
    phoneNumber: '',
    interest: 'Buying Residential Property'
  });
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', email: '', country: '', phoneNumber: '', interest: 'Buying Residential Property' });
    } catch (error) {
  ...
  // --- DATA ARRAYS ---
  const opportunities = [
    { title: "Residential Property", desc: "Invest in premium apartments, luxury villas, and plotted developments in India's fastest-growing urban corridors.", icon: <Home className="w-8 h-8 text-[#FF6600]" /> },
    { title: "Commercial Real Estate", desc: "High-yield investments in Grade-A office spaces, retail hubs, and IT parks generating steady, long-term rental income.", icon: <Building2 className="w-8 h-8 text-[#FF6600]" /> },
    { title: "Real Estate Funds & REITs", desc: "Access the real estate market without direct property management through Real Estate Investment Trusts and AIFs.", icon: <PieChart className="w-8 h-8 text-[#FF6600]" /> },
    { title: "Fractional Real Estate", desc: "Co-own premium commercial assets with a smaller ticket size, earning proportionate high-yield rentals and capital appreciation.", icon: <Coins className="w-8 h-8 text-[#FF6600]" /> }
  ];

  const howWeSupport = [
    { title: "Real Estate Investment Advisory", desc: "We analyze your global portfolio and recommend strategic real estate assets that perfectly align with your wealth creation goals." },
    { title: "Market Research & Evaluation", desc: "Our team conducts rigorous due diligence, developer background checks, and micro-market analysis to ensure safe investments." },
    { title: "Documentation & Compliance", desc: "We handle the complex NRI property laws, FEMA regulations, RERA compliance, and legally bulletproof Specific Powers of Attorney." },
    { title: "End-to-End Transaction Support", desc: "From initial site-visits (virtual or physical) and negotiation, to final registry, lower TDS certificates, and ultimate fund repatriation." }
  ];
            ...
            <form className="p-8 space-y-4" onSubmit={handleModalSubmit}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={formData.country} onChange={(e) => setFormData({...formData, country: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#FF6600] outline-none" placeholder="e.g. UK, UAE" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                  <input type="tel" required value={formData.phoneNumber} onChange={(e) => setFormData({...formData, phoneNumber: e.target.value})} className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#FF6600] outline-none" placeholder="+1 234 567 890" />
                </div>
              </div>
              <div>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-xl mt-4 flex justify-center items-center transition shadow-lg ${isSubmitting ? 'bg-gray-400' : 'bg-[#FF6600] hover:bg-[#e55c00]'}`}>
                {isSubmitting ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <>Schedule Advisory Call <Send className="w-5 h-5 ml-2" /></>}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-w-5xl mx-auto text-gray-800 flex flex-col md:flex-row">
            <div className="p-8 md:p-12 md:w-1/2 border-b md:border-b-0 md:border-r border-gray-200 bg-slate-50">
              <div className="mb-8">
                ...
                <input type="range" min="5000000" max="100000000" step="1000000" value={propertyValue} onChange={(e) => setPropertyValue(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
              </div>
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                ...
                <input type="range" min="3" max="25" step="1" value={holdingPeriod} onChange={(e) => setHoldingPeriod(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="range" min="3" max="15" step="0.5" value={appreciationRate} onChange={(e) => setAppreciationRate(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
                </div>
                <div>
                  <div className="flex justify-between items-center mb-4">
                  ...
                  <input type="range" min="0" max="10" step="0.5" value={rentalYield} onChange={(e) => setRentalYield(Number(e.target.value))} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#003366]" />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div key={index} className="bg-slate-50 border border-gray-200 rounded-2xl overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md">
                <button onClick={() => setActiveFaq(activeFaq === index ? null : index)} className={`w-full flex justify-between items-center p-6 md:p-8 transition text-left ${activeFaq === index ? 'bg-white' : 'hover:bg-gray-100'}`}>
                ...
                <div className="transition-all duration-500 ease-in-out overflow-hidden bg-white" style={{ maxHeight: activeFaq === index ? '300px' : '0', opacity: activeFaq === index ? 1 : 0 }}>
