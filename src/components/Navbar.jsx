        ...
        <div className="px-4 pt-4 pb-8 space-y-2 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            <div className="flex justify-between items-center border-b border-gray-50">
              <Link to="/services" className="px-4 py-3 text-[#003366] font-bold flex-1">Our Services</Link>
              <button onClick={() => setIsServicesOpen(!isServicesOpen)} className="p-3 text-[#003366]"><ChevronDown className={`w-5 h-5 transition-transform ${isServicesOpen ? 'rotate-180' : ''}`} /></button>
            </div>
            <div className={`bg-slate-50 overflow-hidden transition-all duration-300 ${isServicesOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
              {serviceLinks.map((service, index) => (
                <Link key={index} to={service.path} className="block px-8 py-3 text-sm text-gray-600 font-medium hover:text-[#FF6600] border-b border-gray-100/50">{service.name}</Link>
              ))}
            </div>
          </div>
          <div>
            <div className="flex justify-between items-center border-b border-gray-50">
              <div className="px-4 py-3 text-[#FF6600] font-bold flex-1">NRI Invest</div>
              <button onClick={() => setIsNriOpen(!isNriOpen)} className="p-3 text-[#FF6600]"><ChevronDown className={`w-5 h-5 transition-transform ${isNriOpen ? 'rotate-180' : ''}`} /></button>
            </div>
            <div className={`bg-orange-50/50 overflow-hidden transition-all duration-300 ${isNriOpen ? 'max-h-[300px]' : 'max-h-0'}`}>
              {nriLinks.map((nri, index) => (
                <Link key={index} to={nri.path} className="block px-8 py-3 text-sm text-[#003366] font-bold hover:text-[#FF6600] border-b border-orange-100/50">{nri.name}</Link>
              ))}
            </div>
          </div>
